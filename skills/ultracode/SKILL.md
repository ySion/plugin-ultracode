...
driven by `codex exec` subprocesses. For imperative multi-agent control flow there is also a
**Workflow scripts** runner (`ultracode-cli.js script`) that binds those primitives into a plain
async-JavaScript scope — see _Workflow scripts_ below (it runs arbitrary in-process Node, so only run trusted
scripts).

## Workflow

1. Use the Ultracode CLI when a parallel worker pass is worth the overhead: `node scripts/ultracode-cli.js
   {plan|run|pipeline|resume|status|script} [--flags]`.
2. Prefer `run` for a bounded fan-out/fan-in pass. Reach for `pipeline` when stages must
   feed each other (a `depends_on` DAG with `{{steps.<id>.output}}` data flow, verify/loop stages).
3. Keep workers read-only unless the user explicitly asks for writable child Codex runs.
4. Treat worker failures as real failures. Do not replace them with guessed output.
5. Synthesize worker results in the parent Codex thread, then perform any edits yourself so the app/TUI keeps
   the meaningful implementation visible.
6. If the task is small, skip Ultracode and work directly.

## CLI Commands

Run commands as `node scripts/ultracode-cli.js <command> [--flags]` from the plugin checkout, or use the
absolute CLI path injected by the prompt hook when the plugin is installed from cache.

- `plan`: produce the worker plan without running subprocesses.
- `run`: run Codex subprocess workers in parallel and return structured findings.
- `pipeline`: run a declarative `steps[]` DAG of Codex stages (the barrier-free `runPipeline` /
  `runParallel` parity path) — see _pipeline arguments_ below.
- `resume`: resume a persisted workflow — completed steps are reused from the journal, only
  missing/failed/forced steps re-run.
- `status`: inspect the latest persisted workflow state (now journaled, so it reflects mid-flight
...
- `script`: run an imperative Workflow script (the Codex analogue of Claude Code's Workflow tool). It runs
  arbitrary in-process Node with full host privileges (NOT a sandbox). See _Workflow scripts_ below.
...
The result is the same journaled record shape as `run`, so `status` and `resume`
...
engine option names map to kebab-case flags (e.g. `--budget-tokens`, `--max-retries`, `--reasoning-effort`,
...
become `record.result`. The result is a journaled `kind: "script"` record readable by `status`.
